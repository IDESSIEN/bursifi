...
interface UniData {
  ...
  status: string; adminName: string; adminEmail: string; walletStatus: string;
  wallet: string; programs: string[]; terms: Term[];
  fees: { faculty: string; level: string; amount: number; currency: string }[];
  enrollmentCount: number; joinedDate: string;
}

const SEED_UNIS: UniData[] = [
  {
...
function WalletApprovalModal({ onClose, onApprove }: { onClose: () => void, onApprove: (wallet: string) => void }) {
  ...
  const simulateSign = () => { setSigning(true); setTimeout(() => { setSigning(false); setStep(2); }, 2000); };
  return (
        ...
        <div style={{ padding:"16px 24px", borderBottom:`1px solid ${T.border}` }}>
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            {WALLET_APPROVAL_STEPS.map((s, i) => (
              <div key={s.id} style={{ display:"flex", alignItems:"center", flex: i < 4 ? 1 : "none" as unknown as number }}>
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                  <div className={`step-node ${i < step ? "done" : i === step ? "active" : "pending"}`} style={{ width:24, height:24, fontSize:11 }}>{i < step ? "✓" : i+1}</div>
        ...
        <div style={{ padding:"24px" }}>
          {step === 0 && (
            <div className="fade-up">
              <div className="warning-box" style={{ marginBottom:20 }}>
                ...
                <div style={{ fontSize:13, color:T.text, lineHeight:1.7 }}>This wallet address is where all semester fee escrow releases will be sent. Triple-check this is the correct university treasury wallet before continuing.</div>
              </div>
              <div style={{ marginBottom:16 }}>
                <label className="field-label">Treasury Wallet Address</label>
                <input className={`dark-input ${walletAddr && !isValidAddr ? "error" : ""}`} placeholder="0x..." value={walletAddr} onChange={e => setWalletAddr(e.target.value)} style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:13 }} />
                {walletAddr && !isValidAddr && <span className="field-error">Must be a valid 42-character Ethereum address starting with 0x</span>}
                <span className="field-hint">Paste the exact address from your institution treasury. Arc Network only.</span>
              </div>
              <div style={{ marginBottom:20 }}>
                <label className="field-label">Confirm Wallet Address</label>
                <input className={`dark-input ${confirmAddr && !addressesMatch ? "error" : ""}`} placeholder="Re-enter 0x..." value={confirmAddr} onChange={e => setConfirmAddr(e.target.value)} style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:13 }} />
              ...
              <button className="btn-primary" style={{ width:"100%" }} disabled={!isValidAddr || !addressesMatch} onClick={() => setStep(1)}>Continue to Signature Verification →</button>
            </div>
          )}
          {step === 1 && (
            <div className="fade-up">
              <div className="info-box" style={{ marginBottom:20 }}>
                ...
                <div style={{ fontSize:13, color:T.text, lineHeight:1.7 }}>To confirm you control this wallet, sign a short message. This is gasless and costs nothing.</div>
              </div>
              <div style={{ background:"rgba(255,255,255,0.03)", border:`1px solid ${T.border}`, borderRadius:12, padding:"14px 16px", marginBottom:20 }}>
                ...
                <div style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:12, color:T.arc, lineHeight:1.7 }}>
                  {`"I confirm that ${walletAddr.slice(0,10)}...${walletAddr.slice(-6)} is the official treasury wallet of my institution. Timestamp: ${new Date().toISOString()}"`}
                </div>
              </div>
          ...
          {step === 2 && (
            <div className="fade-up" style={{ textAlign:"center" }}>
              ...
              <div style={{ fontSize:14, color:T.muted, marginBottom:24, lineHeight:1.7 }}>The EduCredit ops team will verify this wallet. This typically takes 1-2 business days.</div>
              <div className="warning-box" style={{ textAlign:"left", marginBottom:20 }}>
                <div style={{ fontSize:13, color:T.warn, lineHeight:1.7 }}>Do not change or resubmit the wallet address while review is in progress.</div>
              </div>
              <button className="btn-primary" style={{ width:"100%" }} onClick={() => setStep(3)}>Simulate: Review Complete →</button>
            </div>
          )}
          {step === 3 && (
            <div className="fade-up">
              <div className="info-box" style={{ marginBottom:20 }}>
                ...
                <div style={{ fontSize:13, color:T.text, lineHeight:1.7 }}>A second authorised administrator must confirm this wallet address.</div>
              </div>
              <div style={{ marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                ...
                <button className="btn-primary" style={{ flex:1 }} disabled={!confirmName} onClick={() => { setStep(4); setTimeout(() => { onApprove(walletAddr); onClose(); }, 1500); }}>Confirm & Activate Wallet →</button>
              </div>
            </div>
          )}
        </div>
...
function FeeScheduleEditor({ fees, setFees }: { fees: FeeRow[], setFees: (f: FeeRow[] | ((prev: FeeRow[]) => FeeRow[])) => void }) {
  ...
  return (
    <div>
      ...
      {fees.length === 0 ? (
        <div style={{ textAlign:"center", padding:"28px", background:"rgba(255,255,255,0.02)", border:`1px dashed ${T.border}`, borderRadius:12, color:T.muted, fontSize:14 }}>No fee rows yet. Click &ldquo;+ Add Row&rdquo; to begin.</div>
      ) : (
...
function TermEditor({ terms, setTerms }: { terms: Term[], setTerms: (t: Term[] | ((prev: Term[]) => Term[])) => void }) {
  ...
  return (
    <div>
      ...
      {terms.length === 0 ? (
        <div style={{ textAlign:"center", padding:28, background:"rgba(255,255,255,0.02)", border:`1px dashed ${T.border}`, borderRadius:12, color:T.muted, fontSize:14 }}>No terms yet. Click &ldquo;+ Add Term&rdquo; to begin.</div>
      ) : (
          ...
          {terms.map((term, i) => (
            <div key={i} style={{ background:"rgba(255,255,255,0.02)", border:`1px solid ${T.border}`, borderRadius:12, padding:"16px" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
                {([["start","Semester Start"],["end","Semester End"],["releaseDate","Escrow Release Date"]] as [keyof Term, string][]).map(([key, label]) => (
                  <div key={key}>
...
function EnrollmentUploader() {
  ...
  return (
    <div>
      ...
      <div className="field-hint" style={{ marginBottom:12 }}>Upload a CSV with columns: matric_number, full_name, faculty, program, level, semester, email.</div>
      {!uploaded ? (
        ...
        <div className="fade-up">
          ...
          <div style={{ background:"rgba(255,255,255,0.02)", border:`1px solid ${T.border}`, borderRadius:12, overflow:"hidden" }}>
            <div style={{ padding:"10px 16px", borderBottom:`1px solid ${T.border}`, fontSize:11, fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", color:T.muted }}>Preview (first 5 rows)</div>
...
interface FormData { name:string; country:string; city:string; website:string; accreditationBody:string; adminName:string; adminTitle:string; adminEmail:string; adminPhone:string; }

function RegistrationForm({ onComplete }: { onComplete: (data?: unknown) => void }) {
  const [step, setStep] = useState(0);
  const [form, setForm] = useState<FormData>({ name:"", country:"", city:"", website:"", accreditationBody:"", adminName:"", adminTitle:"", adminEmail:"", adminPhone:"" });
  ...
  const STEPS = [
    { label:"Institution", icon:"🏛️" },{ label:"Admin", icon:"👤" },{ label:"Wallet", icon:"🔐" },
    { label:"Fees", icon:"💰" },{ label:"Calendar", icon:"📅" },{ label:"Students", icon:"🎓" },{ label:"Review", icon:"✅" },
  ];
  const handleSubmit = () => {
    setSubmitting(true);
    setTimeout(() => { setSubmitting(false); onComplete({ ...form, fees, terms, walletAddr, faculties, status:"review" }); }, 2000);
  };
  return (
    <div style={{ display:"grid", gridTemplateColumns:"220px 1fr", gap:24, alignItems:"start" }}>
      ...
      <div className="glass-card" style={{ padding:"20px 14px" }}>
        ...
        <div style={{ paddingLeft:6 }}>
          ...
          <div className="progress-track"><div className="progress-fill" style={{ width:`${Math.round((step/(STEPS.length-1))*100)}%` }} /></div>
      ...
      <div className="glass-card">
        <div style={{ padding:"20px 24px", borderBottom:`1px solid ${T.border}`, display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:"rgba(255,209,102,0.1)", border:"1px solid rgba(255,209,102,0.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>{STEPS[step].icon}</div>
          <div>
        ...
        <div style={{ padding:"28px 24px" }}>
          {step === 0 && (
            <div className="fade-up">
              ...
              <button className="btn-primary" disabled={!form.name||!form.country||!form.city} onClick={() => setStep(1)}>Continue →</button>
            </div>
          )}
          {step === 1 && (
            <div className="fade-up">
              <div className="info-box" style={{ marginBottom:20 }}>
                ...
                <div style={{ fontSize:13, color:T.text, lineHeight:1.7 }}>This should be the head of bursary or a senior finance officer with authority to manage fee collection.</div>
              </div>
              <div style={{ display:"grid", gap:16, marginBottom:24 }}>
                {([["adminName","Full Name","e.g. Dr. Emem Bassey"],["adminTitle","Job Title","e.g. Bursar / Head of Finance"],["adminEmail","Official Email","e.g. bursary@uniuyo.edu.ng"],["adminPhone","Phone Number","e.g. +234 801 234 5678"]] as [keyof FormData, string, string][]).map(([key, label, ph]) => (
                  <div key={key}>
                    <label className="field-label">{label}</label>
          ...
          {step === 2 && (
            <div className="fade-up">
              <div className="warning-box" style={{ marginBottom:20 }}>
                ...
                <div style={{ fontSize:13, color:T.text, lineHeight:1.8 }}>All semester escrow releases flow to this wallet. EduCredit requires address confirmation, signature verification, manual review, and a second admin sign-off.</div>
              </div>
              {walletApproved ? (
                <div className="info-box" style={{ marginBottom:20 }}>
                ...
                <button className="btn-primary" style={{ width:"100%", marginBottom:12 }} onClick={() => setShowWallet(true)}>🔐 Begin 5-Step Wallet Approval →</button>
              )}
              <div style={{ display:"flex", gap:10 }}>
          ...
          {step === 3 && (
            <div className="fade-up">
              <div style={{ marginBottom:24 }}><FeeScheduleEditor fees={fees} setFees={setFees} /></div>
              <div style={{ display:"flex", gap:10 }}>
                <button className="btn-ghost" onClick={() => setStep(2)}>← Back</button>
          ...
          {step === 4 && (
            <div className="fade-up">
              <div style={{ marginBottom:24 }}><TermEditor terms={terms} setTerms={setTerms} /></div>
              <div style={{ display:"flex", gap:10 }}>
          ...
          {step === 5 && (
            <div className="fade-up">
              <div style={{ marginBottom:24 }}><EnrollmentUploader /></div>
              <div style={{ display:"flex", gap:10 }}>
          ...
          {step === 6 && (
            <div className="fade-up">
              <div style={{ display:"grid", gap:14, marginBottom:24 }}>
                {[
                  ...
                  <div key={section.label} style={{ background:"rgba(255,255,255,0.02)", border:`1px solid ${T.border}`, borderRadius:12, overflow:"hidden" }}>
                    <div style={{ padding:"10px 16px", borderBottom:`1px solid ${T.border}`, fontSize:11, fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", color:T.gold }}>{section.label}</div>
              ...
              <div className="warning-box" style={{ marginBottom:20 }}>
                <div style={{ fontSize:13, color:T.warn, lineHeight:1.7 }}>Submitting sends your application to the EduCredit ops team for manual verification. Typically 2-3 business days.</div>
              </div>
              <div style={{ display:"flex", gap:10 }}>
...
function UniDetailPanel({ uni, onBack }: { uni: UniData, onBack: () => void }) {
    ...
    <div className="fade-up">
      <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:24 }}>
        ...
        <div style={{ marginLeft:"auto" }}>
          <span className={`badge ${uni.status==="verified"?"badge-verified":"badge-pending"}`}>{uni.status==="verified"?"✓ Verified":"Under Review"}</span>
        </div>
      </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14 }}>
          ...
          <div className="glass-card" style={{ padding:"20px", gridColumn:"1/-1" }}>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
              {([["Admin Contact",uni.adminEmail],["Treasury Wallet",uni.wallet],["Wallet Status",uni.walletStatus]] as [string,string][]).map(([k,v]) => (
                <div key={k}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div>
                  ...
                  <div style={{ display:"flex", gap:24 }}>
                    {([["Start",t.start],["End",t.end]] as [string,string][]).map(([k,v]) => (
                      <div key={k}>
...
export default function UniversityPortal() {
  ...
  const [unis, setUnis] = useState<UniData[]>(SEED_UNIS);
  const [selectedUni, setSelectedUni] = useState<UniData | null>(null);
  const [search, setSearch] = useState("");
  const filtered = unis.filter(u => u.name.toLowerCase().includes(search.toLowerCase()) || u.country.toLowerCase().includes(search.toLowerCase()));
  const handleRegistrationComplete = (data: unknown) => {
    const d = data as Record<string, unknown>;
    const newUni: UniData = {
      id: (d.name as string).replace(/\s+/g,"").toUpperCase().slice(0,8),
      name: d.name as string, country: d.country as string, flag:"🏛️", city: d.city as string,
      status:"review", adminName: d.adminName as string, adminEmail: d.adminEmail as string,
      walletStatus:"pending", wallet: (d.walletAddr as string) || "Pending approval",
      programs: d.faculties as string[], terms: d.terms as Term[], fees: d.fees as UniData["fees"],
        ...
        <header style={{ borderBottom:`1px solid ${T.border}`, backdropFilter:"blur(20px)", background:"rgba(6,7,16,0.85)", position:"sticky", top:0, zIndex:50 }}>
          <div style={{ maxWidth:1200, margin:"0 auto", padding:"0 24px", display:"flex", alignItems:"center", height:64, gap:20 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <div>
                <div style={{ fontWeight:800, fontSize:15, letterSpacing:"-0.02em", color:T.text }}>Bursifi</div>
            ...
            <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:14 }}>
              ...
              {view !== "register" && (
                <button className="btn-primary" style={{ padding:"9px 20px", fontSize:13, width:"auto" }} onClick={() => setView("register")}>+ Register University</button>
              )}
            </div>
          </div>
        </header>
            ...
            <div>
              <div style={{ marginBottom:24, display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:13, color:T.muted, marginTop:2 }}>Complete all 7 steps to go live on Bursifi</div>
                </div>
              </div>
              <RegistrationForm onComplete={handleRegistrationComplete} />
            </div>
          )}
          {view === "detail" && selectedUni && <UniDetailPanel uni={selectedUni} onBack={() => setView("list")} />}
          {view === "list" && (
            <div>
              ...
              <div className="glass-card">
                ...
                <table className="data-table">
                  <thead><tr>{["Institution","Country","Admin Contact","Wallet","Status","Students","Action"].map(h => <th key={h}>{h}</th>)}</tr></thead>
                  <tbody>
                    {filtered.map(u => (
                      <tr key={u.id} style={{ cursor:"pointer" }} onClick={() => { setSelectedUni(u); setView("detail"); }}>
                        ...
                        <td><span className={`badge ${u.walletStatus==="live"?"badge-verified":u.walletStatus==="review"?"badge-review":"badge-pending"}`}>{u.walletStatus==="live"?"✓ Live":u.walletStatus==="review"?"In Review":"Pending"}</span></td>
                        <td><span className={`badge ${u.status==="verified"?"badge-verified":u.status==="review"?"badge-review":"badge-pending"}`}>{u.status==="verified"?"✓ Verified":u.status==="review"?"Under Review":"Pending"}</span></td>
