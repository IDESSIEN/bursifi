...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .dropdown-panel{position:absolute;top:calc(100% + 6px);left:0;right:0;background:#15182A;border:1px solid rgba(255,255,255,0.14);border-radius:16px;z-index:100;overflow:hidden;box-shadow:0 24px 70px rgba(0,0,0,0.7),0 0 0 1px rgba(255,209,102,0.05);animation:fadeUp 0.2s ease both}
  .dropdown-item{padding:11px 16px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;transition:background 0.15s;border-left:2px solid transparent}
  .dropdown-item:hover{background:rgba(255,255,255,0.05);border-left-color:#FFD166}
  .dropdown-item.selected{background:rgba(255,209,102,0.09);border-left-color:#FFD166}
  .dropdown-group-header{padding:8px 16px 4px;font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#6B70A0;background:rgba(0,0,0,0.4);border-bottom:1px solid rgba(255,255,255,0.06)}
  ...
  .gold-line{height:1px;background:linear-gradient(90deg,transparent,rgba(255,209,102,0.5),transparent)}
...
function SendView({onPaymentAdded, address, connectorClient}: {onPaymentAdded: (p: any) => void, address: string | undefined, connectorClient: any}){
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    try {
      const provider = connectorClient?.transport?.value?.provider
        ?? window.ethereum;
    ...
    <div className="fade-up" style={{textAlign:"center",padding:"12px 0 4px"}}>
      ...
      <div style={{background:"rgba(0,229,160,0.04)",border:"1px solid rgba(0,229,160,0.15)",borderRadius:14,padding:"20px 24px",marginBottom:24,textAlign:"left"}}>
        {[["Amount",`${form.amount} USDC`],["Recipient",school?.name],["Student",form.student],["Semester",form.semester],["Bridge",`${CHAINS.find(c=>c.id===form.fromChain)?.name} → Arc`],["Tx Hash",txHash]].map(([k,v])=>(
          <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderBottom:"1px solid rgba(255,255,255,0.06)",fontSize:14}}>
      ...
      {step===0&&(
        <div className="fade-up" ref={dropRef}>
          ...
          <div style={{position:"relative",marginBottom:12}}>
            ...
            />
            {search&&(
              <span onClick={()=>{setSearch("");setOpen(false);}} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",cursor:"pointer",color:"#6B70A0",fontSize:18}}>×</span>
            )}
          </div>
          ...
          {open&&(
            <div style={{background:"#15182A",border:"1px solid rgba(255,255,255,0.14)",borderRadius:16,overflow:"hidden",boxShadow:"0 24px 70px rgba(0,0,0,0.7)",marginBottom:12,maxHeight:360,overflowY:"auto"}}>
              ...
              {Object.keys(grouped).length===0
                ? <div style={{padding:32,textAlign:"center",color:"#6B70A0",fontSize:14}}>No universities found for "{search}"</div>
                : Object.entries(grouped).map(([country,schools])=>(
                  <div key={country}>
                    ...
                    {schools.map(s=>(
                      <div
                        key={s.id}
                        onClick={()=>{setForm(f=>({...f,school:s.id}));setOpen(false);setSearch("");}}
                        ...
                        onMouseLeave={e=>e.currentTarget.style.background=form.school===s.id?"rgba(200,169,110,0.12)":"transparent"}
                      >
      ...
      {step===1&&(
        <div className="fade-up">
          <div style={{display:"grid",gap:16,marginBottom:20}}>
            {[{key:"student",label:"Student Full Name",placeholder:"e.g. Amara Osei"},{key:"note",label:"Optional Note",placeholder:"e.g. 'From Dad, London'"}].map(({key,label,placeholder})=>(
      ...
      {step===2&&(
        <div className="fade-up">
          ...
          <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,overflow:"hidden",marginBottom:16}}>
            ...
            {[["To",school?.name],["Student",form.student],["Semester",form.semester],["Amount",`${form.amount} USDC`],["Bridge",`${CHAINS.find(c=>c.id===form.fromChain)?.name} → Arc`],["Protocol","Circle CCTP"],["Finality","< 1 second"],["Gas Fee","~0.01 USDC"]].map(([k,v])=>(
...
function DashboardView({payments}){
      ...
      <div className="glass-card">
        ...
        <div style={{overflowX:"auto"}}>
          <table className="data-table">
            <thead>
              <tr>{["Date","Student","School","Amount","Route","Tx Hash","Status"].map(h=><th key={h}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {[...payments].reverse().map(p=>(
                <tr key={p.id}>
...
export default function Bursifi(){
  ...
  const handlePaymentAdded=(p)=>setPayments(prev=>[...prev,p]);
        ...
        <header style={{borderBottom:"1px solid rgba(255,255,255,0.06)",backdropFilter:"blur(20px)",background:"rgba(7,8,10,0.85)",position:"sticky",top:0,zIndex:50}}>
          <div style={{maxWidth:1080,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",height:64}}>
            ...
            <nav style={{display:"flex",gap:4,flex:1}}>
              {NAV.map((label,i)=><button key={label} className={`nav-tab ${view===i?"active":""}`} onClick={()=>setView(i)}>{label}</button>)}
            </nav>
            <div style={{display:"flex",alignItems:"center",gap:20}}>
              ...
              {!mounted ? (
                <button className="btn-gold" style={{padding:"7px 16px"}}>Connect Wallet</button>
              ) : address ? (
                <button className="btn-gold" style={{padding:"7px 16px"}} onClick={()=>disconnect()}>
                  {address.slice(0,6)}...{address.slice(-4)} ✓
                </button>
              ) : (
                <button className="btn-gold" style={{padding:"7px 16px"}} onClick={()=>connect({connector:connectors[0]})}>
                  Connect Wallet
                </button>
              )}
            </div>
          ...
          {view===1&&(
            <div>
              <div style={{marginBottom:24}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,letterSpacing:"0.04em",color:"#F0EDE8"}}>PAYMENT DASHBOARD</div>
                <div style={{color:"#5A5E6B",fontSize:14,marginTop:4}}>Track all your school fee payments across Africa</div>
              </div>
              <DashboardView payments={payments}/>
            </div>
          )}
          {view===2&&(
            <div>
              <div style={{marginBottom:24}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,letterSpacing:"0.04em",color:"#F0EDE8"}}>TRANSACTION HISTORY</div>
                <div style={{color:"#5A5E6B",fontSize:14,marginTop:4}}>Full on-chain record of all fee payments</div>
              </div>
